...
tab empstatusage32, m 	
tab occeduc_max, m		

tabstat empstatusage32, by(sex) missing stat(mean sd n)
tab empstatusage32 sex, m // 867 male 608 female
tab occeduc_max sex, m	// 610 male 458 female

* Drop cases where employment history at 32 is missing

drop if empstatusage32 == .   // 1475 dropped, 867 male drop
*tab occeduc_max sex, m	// 56 male 50 female
*drop if occeduc_max == .	// 106 drop, 56 male dropped
save 	"$temp/predrop2", replace
	*drop if education_new == . // 5 obs
	*drop if citizenship == .	// 1 obs

*save 	"$temp/predrop", replace
